...
export const InsightsView: React.FC<InsightsViewProps> = ({ documents, initialDocId }) => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const answer = await api.chatWithDocument(selectedDocId, input);
  ...
  const handleRegenerateSummary = async () => {
    ...
    try {
      const response = await api.regenerateSummary(selectedDocId);
      // Update the document in the local state
      if (selectedDoc) {
        selectedDoc.summary = response.summary;
        selectedDoc.category = response.category;
        selectedDoc.validity = response.validity;
      }
